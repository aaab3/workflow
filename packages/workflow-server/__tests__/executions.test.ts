...
describe("POST /api/workflows/:id/execute", () => {
  ...
  it("should allow safe terminal commands on workflow server", async () => {
    const id = await makeTestWorkflow({
      nodes: [
        {
          id: "n1",
          ...
          data: {
            label: "Term",
            config: { command: process.platform === "win32" ? "echo hello" : "echo hello", shell: true },
            inputs: [],
            outputs: [{ id: "stdout", name: "stdout", type: "string" }],
          },
        },
      ],
    });
