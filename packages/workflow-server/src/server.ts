...
import { seedDefaultWorkflows } from "./seed/default-workflows.js";
...
export async function createServer(options: ServerOptions = {}) {
  ...
  const seeded = await seedDefaultWorkflows(storage);
  if (seeded > 0 && !silent) {
    console.log(`  Seeded ${seeded} default workflow template(s)`);
  }
