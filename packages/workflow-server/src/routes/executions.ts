...
export async function executionRoutes(
  ...
  const securityConfig = createDefaultSecurityConfig(workflowDir);
      ...
      return reply.send({
        ...
        nodeStates: Object.fromEntries(
          Array.from(context.nodeStates.entries()).map(([id, state]) => [
            id,
            { status: state.status, startTime: state.startTime, endTime: state.endTime, retryCount: state.retryCount, error: state.error },
          ])
        ),
