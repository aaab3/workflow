...
/**
 * Security config for the local workflow server (127.0.0.1).
 * Enables terminal execution so users can run detected CLI agents.
 */
export function createWorkflowServerSecurityConfig(basePath?: string): SecurityConfig {
  const config = createDefaultSecurityConfig(basePath);
  config.terminal = {
    ...config.terminal,
    enabled: true,
    allowShell: true,
