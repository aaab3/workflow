...
export class WorkflowEngine {
    ...
    while (attempt <= maxRetries) {
      ...
      try {
        ...
        if (willRetry) {
          ...
          const retryLog = context.logs[context.logs.length - 1];
          if (retryLog) {
            this.emit({
              type: "log",
              nodeId: retryLog.nodeId,
              level: retryLog.level,
              message: retryLog.message,
              timestamp: retryLog.timestamp,
            });
          }
  ...
  private collectFinalOutputs(_graph: Graph, context: ExecutionContext): Record<string, unknown> {
    const outputs: Record<string, unknown> = {};
    for (const [id, state] of context.nodeStates) {
