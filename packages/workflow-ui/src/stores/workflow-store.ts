...
import type { Workflow, WorkflowNode, WorkflowEdge, PortDef } from "../api/client";
import { getPortType, portsCompatible } from "../utils/port-types";
...
export interface WorkflowState {
  ...
  setNodeExecutionStatus: (nodeId: string, status: NodeExecutionStatus | undefined) => void;
...
export const useWorkflowStore = create<WorkflowState>((set, get) => ({
  ...
  isValidConnection: (connection) => {
    ...
    const sourceOutputs = (source.data.outputs as PortDef[] | undefined) ?? [];
    const targetInputs = (target.data.inputs as PortDef[] | undefined) ?? [];
    const outType = getPortType(sourceOutputs, connection.sourceHandle);
    const inType = getPortType(targetInputs, connection.targetHandle);

    if (!portsCompatible(outType, inType)) return false;
