...
interface SchemaProperty {
  ...
  default?: unknown;
  enum?: string[];
  examples?: string[];          // Suggested values (free-input + datalist)
  minimum?: number;
  maximum?: number;
  format?: string;
...
function renderControl(
  ...
  // String with long description or "content"/"prompt" → Multiline textarea
  if (/prompt|content|body|message|template/i.test(String(property.description ?? "") + name)) {
    return (
      <textarea
        value={String(value ?? "")}
        onChange={(e) => onChange(e.target.value)}
...
import { api, type CredentialSummary, isCredentialRef, makeCredentialRef } from "../api/client";
